...
export default function Providers({ children }) {
  ...
  useEffect(() => {
    console.log("pathname", pathname, "breadcrumbs", breadcrumbs);
    if (breadcrumbs[breadcrumbs.length - index] !== pathname) {
      setBreadcrumbs([...breadcrumbs, pathname]);
    }
  }, [pathname, breadcrumbs, index]);

  return (
    <DarkModeContext.Provider value={{ dark, setDark }}>
      <BreadcrumbsContext.Provider
        value={{ breadcrumbs, setBreadcrumbs, index, setIndex }}
      >
        <>
          {dark === "loading" ? (
